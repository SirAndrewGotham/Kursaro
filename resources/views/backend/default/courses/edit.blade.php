            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple>
                    @foreach($categories as $id => $category)
                        <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $course->categories->contains($id)) ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('course_features') ? 'is-invalid' : '' }}" name="course_features[]" id="course_features" multiple>
                    @foreach($course_features as $id => $course_feature)
                        <option value="{{ $id }}" {{ (in_array($id, old('course_features', [])) || $course->course_features->contains($id)) ? 'selected' : '' }}>{{ $course_feature }}</option>
                    @endforeach
                </select>
